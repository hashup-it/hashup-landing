...
export const CommunityButtons: FC <ICommunityButtons> = ({ name, size, description, icon}) => {
  return (
    <Flex
    w='25%'
    h='142px'
    ...
    >
        <Flex direction='column'>
            ...
            <Flex gridGap='12px' alignItems='center'>
                {icon}
                <Text color='white' fontSize='32px' fontWeight='400' lineHeight='160%'>{name}</Text>
            </Flex>
            <RightArrowIcon/>
        </Flex>
        </Flex>
        <Text color={Colors.subGrey} fontSize='12px' fontWeight='400' lineHeight='140%'>{description}</Text>
    </Flex>
  )
}
