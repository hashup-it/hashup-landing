...
const handleOnScroll = (animationHookReference: any, setCurrentAssetIndex: React.Dispatch<React.SetStateAction<number>>) => {
    if (!animationHookReference) {
        return
    }

const { offsetTop } = animationHookReference.current
const animationBoundEnd = offsetTop - window.scrollY
...
const imageUriArray = getImageUri();
   ...
   useEffect(() => {
    ...
    window.addEventListener('scroll', () => handleOnScroll(animationHookReference, setCurrentAssetIndex), { passive: true });

    return () => window.removeEventListener('scroll', () => handleOnScroll(animationHookReference, setCurrentAssetIndex));
  }, [])



return (
    <Flex position="absolute" top="110vh" right="0" zIndex="-1">
        {<Image src={imageUriArray[currentAssetIndex]} position="sticky" />}
    </Flex>
  )
}