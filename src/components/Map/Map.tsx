...
const Map: NextPage = () => {
  ...
  useEffect(() => {
    ...
    if (typeof map1.current === "string" || map1.current instanceof HTMLElement) {
      ...
      mapboxgl.accessToken = "pk.eyJ1IjoicHJvc3NhIiwiYSI6ImNsZGNid2Y1MzA5NzQzbnE1b2w0NWt3cnAifQ.ZR0ZY3eBKsD7MU100juXbQ"
  ...
  return (
    <Flex p="6rem 0 0 0" flexDirection="column" textAlign="center" gap="20px" >

      <Text fontSize={['32px', '46px', '64px', '70px']} fontWeight="700">See all of the stores<strong>!</strong></Text>

      <Flex h="80vh" w="100vw" textAlign="center" position="relative" justifyContent="end">

        <StoreInfo storeName={storeInfo?.name} logo={storeInfo?.logo}
          city={storeInfo?.city} country={storeInfo?.country} description={storeInfo?.description}
          storeLink={storeInfo?.link} display={leftSectionVisible} marketplace={ storeInfo?.marketplace } />
        
        <Flex h="80vh" w={{ base: '100vw', xl: leftSectionVisible ? "70vw" : "100vw" }} transition="0.2s">
          
          <Flex w="100%" h="100%" color="black" ref={map1} />
